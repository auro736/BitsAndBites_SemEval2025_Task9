...
def main():
    ...
    save_model_dir = f"./ST1/saved_models/{model_name}_{timestamp}"
    os.makedirs(save_model_dir, exist_ok=True)
   
    performance_file = f"./ST1/performance/performance.txt"
    os.makedirs(os.path.dirname(performance_file), exist_ok=True)

    num_labels_hazard_cat = 10  
    num_labels_product_cat = 22  
    ...
    config.num_labels_task1 = num_labels_hazard_cat
    config.num_labels_task2 = num_labels_product_cat

    # prepare data for training, validation and testing
    train_texts, val_texts, test_texts, \
    train_labels_hazard_cat, val_labels_hazard_cat, test_labels_hazard_cat, \
    train_labels_product_cat, val_labels_product_cat, test_labels_product_cat = prepare_data(df_train_path, df_validation_path, df_test_path, mode)

    
    # initialize datasets and dataloaders
    train_dataset = TextClassificationDataset(train_texts, train_labels_hazard_cat, train_labels_product_cat, tokenizer, max_length=512)
    val_dataset = TextClassificationDataset(val_texts, val_labels_hazard_cat, val_labels_product_cat, tokenizer, max_length=512)
    test_dataset = TextClassificationDataset(test_texts, test_labels_hazard_cat, test_labels_product_cat, tokenizer, max_length=512)
    ...
    # Run training
    avg_train_loss, train_score, avg_val_loss, val_score = train(  
                                                                model=model, 
