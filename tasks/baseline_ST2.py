...
def main():
    ...
    output_dir = "./ST2/out_baseline/saved_models"
    ...
    cat_labels = train_df[['hazard', 'product']].values.tolist()
    ...
    logging_dir="./ST2/out_baseline/logs",
    learning_rate = lr,
    ...
    preds_validation = predict_and_save(device, model, tokenizer, valid_texts, "./ST2/out_baseline/predictions/st2_baseline_preds_valid.csv", mlb_categories)
    preds_test = predict_and_save(device, model, tokenizer, test_texts, "./ST2/out_baseline/predictions/st2_baseline_preds_test.csv", mlb_categories)

    score_validation = compute_score(
            true_validation['hazard'].to_numpy(), 
            true_validation['product'].to_numpy(), 
            preds_validation['hazard'].to_numpy(), 
            preds_validation['product'].to_numpy()
            )
    
    score_test = compute_score(
            true_test['hazard'].to_numpy(), 
            true_test['product'].to_numpy(), 
            preds_test['hazard'].to_numpy(), 
            preds_test['product'].to_numpy()
            )
...
def predict_and_save(device, model, tokenizer, predict_texts, output_csv, mlb_categories, max_length=512):
    ...
    pred_df = pd.DataFrame(predictions, columns=["hazard", "product"])
